<!-- From https://animation-svelte.vercel.app/magic/particles -->
...
<script lang="ts">
    ...
    // FIX: Turn this into a reactive declaration so it recalculates on color changes
    $: rgb = hexToRgb(color);
    ...
    // Tracks quantity changes to redraw layout, but ignores the runtime color changes
    $: {
        if (canvasRef) {
            drawParticles();
        }
